...
def fight(current_enemy): #current_enemy - какое-то число
    round = randint(1, 2) 
...
def display_player():
    ...
    print(f'Броня поглощает {round((1 - player["armor"]) * 100,2)}% урона')
    print()
...
def display_inventory():
    ...
    print()
    a = input("Напишите название предмета, который хотите использовать: ")
    if items['5']['name'] == a:
        player['luck'] += 7
        print(f'Готово! Теперь шанс нанести критический урон равен {player["luck"]}%')
        player['inventory'].remove(items['5']['name'])
    if items['1']['name'] == a:
        player['hp'] *= 1.2
        print(f'Готово! Теперь уровень вашего здоровья равен {player["hp"]}')
        player['inventory'].remove(items['1']['name'])
    if items['3']['name'] == a:
        player['attack'] += 5
        print(f'Готово! Теперь уровень вашеей атаки равен {player["attack"]}')
        player['inventory'].remove(items['3']['name'])
    if items['4']['name'] == a:
        player['hp'] += 50
        print(f'Готово! Теперь уровень вашего здоровья равен {player["hp"]}')
        player['inventory'].remove(items['4']['name'])
